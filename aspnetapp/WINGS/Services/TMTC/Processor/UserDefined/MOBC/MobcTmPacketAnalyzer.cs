...
namespace WINGS.Services
{
  ...
  public class MobcTmPacketAnalyzer : TmPacketAnalyzerBase, ITmPacketAnalyzer
  {
    ...
    public override async Task<bool> AnalyzePacketAsync(TmPacketData data, List<TelemetryPacket> prevTelemetry)
    {
      ...
      if (AnalyzeHeader(STX))
      {
        ...
        {     
          ...
          if (ccsdsPriHeaderPointer == 0b_0111_1111_1111)
          {
            ...
            {
              AddTelemetryList(data.Opid, ccsdsTmPacketbuf, ccsdsdataList, apIdList, packetIdList, realtimeFlagList, TIList);
            ...
            while (readPointer !=  tfPriHeaderLen + mpduPriHeaderLen + mpduPacketZone)
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  AddTelemetryList(data.Opid, statictmPacket[data.Opid].ToArray(), ccsdsdataList, apIdList, packetIdList, realtimeFlagList, TIList);
                  statictmPacket[data.Opid].Clear();
      ...
      return await SetTelemetryListValuesAsync(ccsdsdataList, apIdList, packetIdList, realtimeFlagList, TIList, prevTelemetry);
    }
    ...
    private void AddTelemetryList(string opid, byte[] ccsdstmPacket, List<TmPacketData> ccsdsdataList, List<string> apIdList, List<string> packetIdList, List<bool> realtimeFlagList, List<UInt32> TIList)
    {
      ccsdsdataList.Add(new TmPacketData{ Opid = opid, TmPacket = ccsdstmPacket });
      apIdList.Add(GetApId(ccsdstmPacket));
